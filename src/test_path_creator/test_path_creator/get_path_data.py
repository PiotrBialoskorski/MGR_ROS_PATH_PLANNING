...
class getPose(Node):
    ...
    def send(self):
        ...
        print(f'flaga od data: {self.flagdata}, initial: {self.flaginit}, od goal: {self.flaggoal}')
        if self.flaggoal == 1:
            print("halo?")
        ...
        # if self.flagdata == 1:
        #     print('potiwerdzenie otrzymania mapy i przepisania jej')
        ...
        if ((self.flaggoal == 1) and (self.flaginit == 1) and (self.flagdata == 1)):
            ...
            self.get_logger().info(f'publishing: {self.msg}, flags are, init: {self.flaginit}, goal: {self.flaggoal}, data: {self.flagdata}')
            print('wyslalem dane')
            self.flaginit = 0
            self.flaggoal = 0
            self.flagdata = 0
