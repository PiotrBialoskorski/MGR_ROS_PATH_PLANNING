...
class Vec_tangent_algorithm(Node):
    def __init__(self):
    ...
    def VecField_callback(self, msg : EssentialData):
        ...
        self.Vector_field_tangent_path()

    def Vector_field_tangent_path(self):
        ...
        while res >= 0.1:
                ...
                for j in range(int(-D/self.resolution), int(D/self.resolution)+1):
                    if self.grid[iter_y + i][iter_x + j] == 100:
                        ...
                        if temp_dist <= D:
                            ...
                            tangent = np.array([repel[1], repel[0]])
                            tangent_scale = (0.5 / dist) * ((D - dist) / (D))
                            tangent_vec = tangent_vec + tangent * tangent_scale
            if (((-0.001 <= vec[0] + repel_vec[0] <= 0.001) and (-0.001 <= vec[1] + repel_vec[1] <= 0.001)) or math.sqrt(repel_vec[0]**2 + repel_vec[1]**2) > 10):
                ...
                break
            new_pose = new_pose + vec + repel_vec + tangent_vec
...
def main(args=None):
    ...
    Vec_algs = Vec_tangent_algorithm()
